...
原因：因为模型训练的时候没有指定GPU训练，输入的数据类型与网络参数的类型不符。    
解决方案：将模型指定到GPU上去，同时也在summary中指定调用GPU方法    
~~~python
...
if __name__=="__main__":
    ...
    device = torch.device('cuda:0') # 指定到GPU上运行
    model1.to(device) # 指定到GPU上运行

    summary(model1, input_size=(3, 224, 224), device="cuda") #同时指定device为GPU
~~~
